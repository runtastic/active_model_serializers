...
module ActiveModel
  class Serializer
    ...
    # @api private
    def self.serializer_lookup_chain_for(klass, options = {})
      chain = []
      ...
      chain.push("#{namespace}::#{resource_class_name}") if namespace
    ...
    def self.get_serializer_for(klass, options = {})
      ...
      key = { class: klass }
      key[:namespace] = options[:namespace] if options.key?(:namespace)
      serializers_cache.fetch_or_store(key) do
        # NOTE(beauby): When we drop 1.9.3 support we can lazify the map for perfs.
        serializer_class = serializer_lookup_chain_for(klass, options).map(&:safe_constantize).find { |x| x && x < ActiveModel::Serializer }
